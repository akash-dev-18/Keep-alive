...
//       <nav className="container-page flex h-16 items-center justify-between gap-6" aria-label="Primary navigation">
//         <div className="flex items-center gap-8">
//           <Link href={userId ? '/dashboard' : '/'} className="flex items-center gap-2.5 text-slate-950">
//             <span className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-slate-950 text-white">
//               <Activity className="h-4 w-4" />
//             </span>
//             <span className="text-sm font-semibold tracking-tight">KeepAlive</span>
//           </Link>
...
//       key={item.href}
//       href={item.href}
//       // Hamein inline-flex ke sath whitespace-nowrap lagana hai taaki text tootna band ho jaye
//       className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium transition-colors whitespace-nowrap ${
...
//                   className="relative flex h-9 w-9 items-center justify-center rounded-lg border border-transparent text-slate-500 hover:border-slate-200 hover:bg-slate-50 hover:text-slate-950"
//                   aria-label="Open notifications"
//                 >
//                   <Bell className="h-4 w-4" />
//                   {unreadCount > 0 && <span className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-blue-600" />}
//                 </button>

//                 {bellOpen && (
//                   <div className="absolute right-0 mt-2 w-80 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl shadow-slate-950/5">
//                     <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
//                       <span className="text-sm font-semibold text-slate-950">Notifications</span>
//                       {unreadCount > 0 && (
//                         <button type="button" onClick={handleMarkAllRead} className="text-xs font-medium text-blue-700 hover:text-blue-900">
//                           Mark all read
//                         </button>
//                       )}
//                     </div>
...
//                           <div key={n.id} className={`border-b border-slate-100 px-4 py-3 last:border-0 ${n.isRead ? 'bg-white' : 'bg-blue-50/50'}`}>
//                             <p className="text-sm font-medium text-slate-950">{n.title}</p>
//                             {n.body && <p className="mt-1 text-xs leading-5 text-slate-600">{n.body}</p>}
...
//               {isApp && (
//                 <button type="button" onClick={() => setMobileOpen(!mobileOpen)} className="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-slate-600 md:hidden" aria-label="Toggle navigation">
...
//                 <button type="button" className="btn-secondary hidden sm:inline-flex">Sign in</button>
//               </SignInButton>
...
export default function Navbar() {
  ...
  return (
    // 🎨 Sleek Height (h-14) + Deep Dark Palette Match + High End Glassmorphism
